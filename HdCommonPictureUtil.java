...
public class HdCommonPictureUtil {
	
	/** 日志 **/
	private static Logger logger = LoggerFactory.getLogger(HdCommonPictureUtil.class);
	...
	public static boolean yPic(String first, String second, String out,
		...
		try {
			...
			imageResult.setRGB(0, height, widthTwo, heightTwo,
					imageArraySecond, 0, widthTwo);// 设置右半部分的RGB
	 ...
	 * @return
	 */
	public static boolean resize(String sourcePath, int targetW,
			int targetH) {
		boolean flag = false;
		try{
...
//			if(width > height){
//				double rate = Double.valueOf(width)/Double.valueOf(height);
//				Double newH = targetH /rate;
//				targetH = newH.intValue(); 
//			}else if(height > width){
//				Double newW = targetW*1000 / Math.round(height/width)/1000.0;
//				targetW = newW.intValue();
//			}
			
			BufferedImage outImg = resize(sourceImg, targetW, targetH);
			//outImg = rotateImage(outImg,90);
			String format = sourcePath.substring(sourcePath.lastIndexOf(".")+1);
			String targetPath = sourcePath.substring(0,sourcePath.lastIndexOf("."))+"_s."+format;
		    File outFile = new File(targetPath);
		    ImageIO.write(outImg, format, outFile);// 写图片
		    flag = true;
		}catch(Exception ex){
			logger.error("图片缩放失败！" , ex);
		}
		return flag;
	}
     ...
     */
    public static boolean zoomImage(String src,int w,int h) {
        boolean flag = false;
        try {
            double wr=0,hr=0;
            File srcFile = new File(src);
            String format = src.substring(src.lastIndexOf(".")+1);
            String targetPath = src.substring(0,src.lastIndexOf("."))+"_s."+format;
        	...
        	File outFile = new File(targetPath);
            ImageIO.write((BufferedImage) Itemp,format, outFile); //写入缩减后的图片
            flag = true;
        } catch (Exception ex) {
        	logger.error("图片缩放失败！" , ex);
        }
	...
	public static void main(String[] args) throws IOException {
		zoomImage("/Users/duhao/Desktop/aaa.gif", 300, 300);
	}
